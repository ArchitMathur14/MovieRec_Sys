...
# -----------------------------
st.sidebar.header("🎯 Movie Selection & Filters")

# Search or select movie
search_option = st.sidebar.radio("Choose input method:", ["🔍 Search", "📋 Select from list"])
...
# Filters
st.sidebar.subheader("🎚️ Recommendation Filters")
...
st.sidebar.markdown("---")
st.sidebar.info("💡 **Tip**: This system recommends movies based on plot similarity using TF-IDF and Cosine Similarity.")
...
if selected_movie:
    ...
    with col2:
        if 'vote_average' in df.columns:
            st.metric("⭐ Rating", f"{selected_movie_data['vote_average']:.1f}/10")

    with col3:
        if 'popularity' in df.columns:
            st.metric("🔥 Popularity", f"{selected_movie_data['popularity']:.1f}")

    with col4:
        if 'year' in df.columns and pd.notna(selected_movie_data['year']):
            st.metric("📅 Year", int(selected_movie_data['year']))
                ...
                if recommendations.empty:
                    ...
                    for idx, row in recommendations.iterrows():
                        with st.container():
                            ...
                            with col3:
                                st.metric("🎯 Match", f"{row['similarity_score']*100:.1f}%")
                                if 'popularity' in row and pd.notna(row['popularity']):
                                    st.write(f"🔥 **Pop:** {row['popularity']:.0f}")
    ...
    # Welcome screen
    st.info("👈 Please select or search for a movie from the sidebar to get started!")
    ...
    with col1:
        st.markdown('<div class="metric-card"><h2>🎬</h2><h3>Total Movies</h3><h2>{}</h2></div>'.format(len(df)), unsafe_allow_html=True)

    with col2:
        if 'vote_average' in df.columns:
            avg_rating = df['vote_average'].mean()
            st.markdown('<div class="metric-card"><h2>⭐</h2><h3>Avg Rating</h3><h2>{:.1f}</h2></div>'.format(avg_rating), unsafe_allow_html=True)

    with col3:
        if 'year' in df.columns:
            year_range = f"{int(df['year'].min())}-{int(df['year'].max())}"
            st.markdown('<div class="metric-card"><h2>📅</h2><h3>Year Range</h3><h2>{}</h2></div>'.format(year_range), unsafe_allow_html=True)

    with col4:
        unique_features = len(tfidf.get_feature_names_out())
        st.markdown('<div class="metric-card"><h2>🔤</h2><h3>Features</h3><h2>{}</h2></div>'.format(unique_features), unsafe_allow_html=True)

    st.markdown("---")

    # Sample movies preview
    with st.expander("🎥 Browse Sample Movies", expanded=True):
...
st.markdown("""
    <div style='text-align: center; color: #666; padding: 1rem;'>
        <p>Built with ❤️ using Streamlit | Powered by TF-IDF & Cosine Similarity</p>
